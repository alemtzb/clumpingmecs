...
setupVals = function(dtSp){
    ...
    return(list(ZZ=ZZZ,messSq=messSq,sq=sq,yearmat=yearmat,tx=tx))
}
...
runCTestSampled=function(dtVeros,model='Phase1Model',points=10,paramNum=5,ranEf=NULL){
	...
	hcPoints[,1:yrNum]=(hcPoints[,1:yrNum]*2)-1 # BB between -5 and 5
	hcPoints[,yrNum+1]=(hcPoints[,yrNum+1]*2)-1 # DD between -5 and 5
	hcPoints[,yrNum+2]=((hcPoints[,yrNum+2])*2)-1 # Alpha
	hcPoints[,yrNum+3]=(hcPoints[,yrNum+3]*2)-1 # theta
	hcPoints[,yrNum+4]=(hcPoints[,yrNum+4]*5)-.5 # alfa (values <-0.5  can return nan)
	hcPoints[,yrNum+5]=(hcPoints[,yrNum+5]*4)-2 # cc
	...
	crashParams=list()
	for(p in 1:points){
		paramVeros=list(BB=c(hcPoints[p,1:yrNum]),DD=hcPoints[p,yrNum+1],alphas=hcPoints[p,yrNum+2],theta=hcPoints[p,yrNum+3],alfa=hcPoints[p,yrNum+4],cc=hcPoints[p,yrNum+5])
...
#read data
dtSp=read.csv("Muhphagibbscor.csv")
#clean data
dtSp=cleanDt(dtSp)
