...
runCTest=function(dtVeros,paramVeros,model='ParameterizationSecondPhasePresenceAbsence'){
	obj = MakeADFun(dtVeros, paramVeros, DLL=model,random="ranef") #ranEf=ranEf con efectos aleatorios
	obj$hessian = TRUE #????
	opt = nlminb(start = obj$par, obj = obj$fn, gr = obj$gr,control=list(iter.max=10000,eval.max=10000,rel.tol=exp(-8),x.tol=1.5*exp(-6)))
	return(opt)

}

runCTestSampled=function(dtVeros,model='ParameterizationSecondPhasePresenceAbsence',points=10,paramNum=2,interdepth=dtVals$interdepth){
    ...
    hcPoints[,1]=(hcPoints[,1]*2)-1 # mortality D between -5 and 5
    ...
    crashParams=list()
    for(p in 1:points){
        paramVeros=list(DD=hcPoints[p,1],sigma=hcPoints[p,2],BB2=c(hcPoints[p,3:(2+yrNum)]),BB3=c(hcPoints[p,(3+yrNum):(2+yrNum*2)]),alphas=c(hcPoints[p,(3+yrNum*2):(2+yrNum*2+spNum)]),betas=c(hcPoints[p,(3+(yrNum*2)+spNum):(1+(yrNum*2)+(spNum*2))]),alphas2=rep(0,ncol(dtVals$tx)),ranef=matre[p,])
        ...
        # opt1=bnVerTst(dtVeros$t1,dtVeros$t2,dtVeros$x,dtVeros$y,dtVeros$yearmat,dtVeros$sq,dtVeros$ZZ,lamlst=c(hcPoints[p,1:14]),comp=hcPoints[p,15],m=hcPoints[p,16],theta=hcPoints[p,17],alfa=hcPoints[p,18],cc=hcPoints[p,19])
    }
    ...
    hcPoints[,1]=(hcPoints[,1]*2)-1 # mortality parameter
    hcPoints[,2]=(hcPoints[,2]*2)-1 #Sigma
    hcPoints[,3:(2+yrNum)]=(hcPoints[,3:(2+yrNum)]*2)-1 #parameter b
    hcPoints[,(3+yrNum):(2+yrNum*2)]=(hcPoints[,(3+yrNum):(2+yrNum*2)]*2)-1 #parameter c
    hcPoints[,(3+yrNum*2):(2+yrNum*2+spNum)]=(hcPoints[,(3+yrNum*2):(2+yrNum*2+spNum)]*2)-1 #parameter g
    ...
    for(p in 1:points){
        ...
        paramVeros=list(DD=hcPoints[p,1],sigma=hcPoints[p,2],BB2=c(hcPoints[p,3:(2+yrNum)]),BB3=c(hcPoints[p,(3+yrNum):(2+yrNum*2)]),alphas=c(hcPoints[p,(3+yrNum*2):(2+(yrNum*2)+spNum)]),betas=c(hcPoints[p,(3+(yrNum*2)+spNum):(1+(yrNum*2)+(spNum*2))]),alphas2=c(interdepth),ranef=matre[p,])
...
runBasic=function(dtSp,dtVals,points=100){
	   dtVeros=list(tx=dtVals$tx,txmas=log(dtVals$tx+1),interdepth=dtVals$interdepth,t1=dtSp$t1,t2=dtSp$t2,Dsq=dtSp$Profundidad,Dsq2=(dtSp$Profundidad^2)/1000,x=dtSp$x,y=dtSp$y,sq=dtVals$sq,yearmat=dtVals$yearmat,ZZ=dtVals$ZZ,rainMtrxFull=dtVals$rain,BB=dtVals$BB,square=dtVals$square)
paramVeros=list(DD=1,sigma=1,BB2=rep(1,dim(dtVals$yearmat)[2]),BB3=rep(1,dim(dtVals$yearmat)[2]),alphas=rep(1,ncol(dtVals$tx)),betas=rep(1,ncol(dtVals$tx)-1),alphas2=rep(1,ncol(dtVals$tx)),ranef=rep(0,length(unique(dtVals$square))))#sigma=0,ranef=rep(0,length(unique(dtVals$square))))
    compile("/home/invitado/Escritorio/Ale/capitulo2/int/ParameterizationSecondPhasePresenceAbsence.cpp")
	dyn.load(dynlib("ParameterizationSecondPhasePresenceAbsence"))
	optN=runCTestSampled(dtVeros,points=points)
}

#read data
dtSp=read.csv("BouchointMegasinBulten.csv")
#Clean data
dtSp=cleanDt(dtSp)
#eliminate zeros from data (estimate of a zero will always be a zero)
dtSp=cleanzero(dtSp) 
dtVals=setupVals(dtSp,disp=disp,BBinp=BBinp,"Boucho",lagNum=0) #the third parameter of this function must be the first three letters of the generic name and the first thre letters of the specific epithet of the species. For example Micrhochlo kunthii would be "Mickun". This is to associate the dispersal parameters calculated in the first phase with the parameters being esttimated in this phase for each species 


#run model
runBasic(dtSp,dtVals,points=500)->resul
#save results
saveRDS(resul,"500BouchosinBulten")
