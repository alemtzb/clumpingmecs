...
runCTestSampled=function(dtVeros,model='ParameterizationSecondPhase',points=10,paramNum=3,interdepth=dtVals$interdepth){
    ...
    if(nalphas2==0){ #if the competition with none of the species changes with soil depth
        paramNum=paramNum+((2*yrNum)+(2*spNum)-1) #number of parameters to be estimated
    ...
    hcPoints[,2]=(hcPoints[,2]*2)-1 # Theta 
    hcPoints[,3]=(hcPoints[,3]*2)-1 #Sigma
    hcPoints[,4:(3+yrNum)]=(hcPoints[,4:(3+yrNum)]*2)-1 #parameter b
    hcPoints[,(4+yrNum):(3+yrNum*2)]=(hcPoints[,(4+yrNum):(3+yrNum*2)]*2)-1 #parameter c
    hcPoints[,(4+yrNum*2):(3+yrNum*2+spNum)]=(hcPoints[,(4+yrNum*2):(3+yrNum*2+spNum)]*2)-1 # parameter g
    hcPoints[,(4+yrNum*2+spNum):(2+yrNum*2+spNum*2)]=(hcPoints[,(4+yrNum*2+spNum):(2+yrNum*2+spNum*2)]*2)-1 #facilitation parameter
    matre=matrix(rep(0,points*sqNum),nrow=points,ncol=length(unique(dtVals$square)))

    count=1
    crashParams=list()
    for(p in 1:points){
        paramVeros=list(DD=hcPoints[p,1],theta=hcPoints[p,2],sigma=hcPoints[p,3],BB2=c(hcPoints[p,4:(3+yrNum)]),BB3=c(hcPoints[p,(4+yrNum):(3+yrNum*2)]),alphas=c(hcPoints[p,(4+yrNum*2):(3+yrNum*2+spNum)]),betas=c(hcPoints[p,(4+yrNum*2+spNum):(2+yrNum*2+spNum*2)]),alphas2=rep(0,ncol(dtVals$tx)),ranef=matre[p,])
    ...
    return(list(maxOpt=maxOpt,lstValues=lstValues,crashParams=crashParams,points=hcPoints,RE=matre))
        ...
        paramNum=paramNum+((2*yrNum)+(2*spNum)+nalphas2-1)  
    ...
    hcPoints[,(4+yrNum*2):(3+yrNum*2+spNum)]=(hcPoints[,(4+yrNum*2):(3+yrNum*2+spNum)]*2)-1 # parameter g
    hcPoints[,(4+yrNum*2+spNum):(2+yrNum*2+spNum*2)]=(hcPoints[,(4+yrNum*2+spNum):(2+yrNum*2+spNum*2)]*2)-1 #facilitation parameter
    hcPoints[,(3+yrNum*2+spNum*2):(2+(yrNum*2)+(spNum*2)+nalphas2)]=(hcPoints[,(3+yrNum*2+spNum*2):(2+(yrNum*2)+(spNum*2)+nalphas2)]*2)-1 #parmeter h
    matre=matrix(rep(0,points*sqNum),nrow=points,ncol=length(unique(dtVals$square)))

    count=1
    crashParams=list()
    for(p in 1:points){
        hcPoints[p,(3+yrNum*2+spNum*2):(2+(yrNum*2)+(spNum*2)+nalphas2)]->prealphas2
        for(n in 1:nalphas2){
            interdepth[alphas2[n]]=prealphas2[n]
        }
        paramVeros=list(DD=hcPoints[p,1],theta=hcPoints[p,2],sigma=hcPoints[p,3],BB2=c(hcPoints[p,4:(3+yrNum)]),BB3=c(hcPoints[p,(4+yrNum):(3+yrNum*2)]),alphas=c(hcPoints[p,(4+yrNum*2):(3+yrNum*2+spNum)]),betas=c(hcPoints[p,(4+yrNum*2+spNum):(2+yrNum*2+spNum*2)]),alphas2=c(interdepth),ranef=matre[p,])
...
paramVeros=list(DD=1,theta=1,sigma=0,BB2=rep(1,dim(dtVals$yearmat)[2]),BB3=rep(1,dim(dtVals$yearmat)[2]),alphas=rep(1,ncol(dtVals$tx)),betas=rep(1,ncol(dtVals$tx)-1),alphas2=rep(1,ncol(dtVals$tx)),ranef=rep(0,length(unique(dtVals$square))))#sigma=0,ranef=rep(0,length(unique(dtVals$square))))
	compile("/clumpingmecs/phase 2/ParameterizationSecondPhase.cpp")
	dyn.load(dynlib("ParameterizationSecondPhase"))
...
#save results
saveRDS(resul,"BultenintMegasinDalserTripurMuhrigFloped3")
